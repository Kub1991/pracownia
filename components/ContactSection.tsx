  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const webhookData = {
        ...
        problemsText: selectedProblems
          .map((id) => problemOptions.find((option) => option.id === id)?.label)
          .join(', '),
      ...
      const response = await fetch(
        'https://hook.eu2.make.com/eon9x3dqgv1bj83fo6hft8qtca2f4y5t',
        {
          method: 'POST',
          headers: {
                ...
                <Card className="bg-gray-50 rounded-2xl lg:rounded-3xl p-4 sm:p-6 lg:p-8 border border-gray-200 shadow-none">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                        {problemOptions.map((problem) => (
                          <Card
                            ...
                            onClick={() =>
                              submitStatus !== 'submitting' && handleProblemToggle(problem.id)
                            }
                          >
                            <div className="flex items-center gap-2 sm:gap-3">
                              ...
                              >
                                <div className="scale-75 sm:scale-100">{problem.icon}</div>
                              </div>
                              <span className="text-xs sm:text-sm font-medium">{problem.label}</span>
                            </div>
                          </Card>
                        ))}
                      </div>
                    ...
                    <div className="flex items-start gap-2 sm:gap-3">
                      ...
                      <Label
                        htmlFor="consent"
                        className="text-xs sm:text-sm text-clr-dark/80 leading-relaxed cursor-pointer"
                      >
